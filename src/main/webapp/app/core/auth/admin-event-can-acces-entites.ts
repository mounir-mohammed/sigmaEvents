...
export class AdminEventCanAccesEntities implements CanActivate {
    ...
    return this.accountService.identity().pipe(
      map(account => {
        const hasProperty =
          ...
          (account?.printingCentre?.event != null && this.accountService.hasAnyAuthority([Authority.EVENT_ADMIN]));
        if (hasProperty) {
          return true;
        } else {
