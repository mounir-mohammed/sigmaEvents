...
export class UserEventCanAccesEntities implements CanActivate {
      ...
      map(account => {
        const hasProperty =
          ...
          (account?.printingCentre?.event != null &&
            (this.accountService.hasAnyAuthority([Authority.EVENT_USER]) || this.accountService.hasAnyAuthority([Authority.EVENT_ADMIN])));
        if (hasProperty) {
          return true;
        } else {
