package ma.sig.events.config.timezone;
...
import ma.sig.events.repository.timezone.DateTimeWrapper;
import ma.sig.events.repository.timezone.DateTimeWrapperRepository;
...
class HibernateTimeZoneIT {
    ...
    private String generateSqlRequest(String fieldName, long id) {
        return format("SELECT %s FROM sigma_date_time_wrapper where id=%d", fieldName, id);
    }
